...
Stato 0.1.61: **216 test in 29 suite, 4557 asserzioni, tutto verde**, più i 3
...
| `unit/test_destiny_evaluator.gd` | 8 | la scala completa Minimum → Victory → Triumph, la cumulatività (perdere il Minimum azzera il livello), il conteggio delle Scoperte, un risultato per ogni posto, l'evidenza che registra condizioni ed Echo, i conti rimasti aperti come dati, il Destino condivisibile risolto su chi lo giura, e i due livelli dove una pietra o una cicatrice spostano il gradino |
| `unit/test_destiny_choice.gd` | 3 | la spina e la scelta (D-167): una `some_of` tiene al conto che chiede e non a uno di meno, la prova apre la scelta strada per strada, e — il difetto che la seduta ha quasi introdotto — un seggio vede ancora quello che vuole quando la clausola sta **dentro** una scelta |
